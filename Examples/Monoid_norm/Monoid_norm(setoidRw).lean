...
def app'  {α}
  := Quotient.map₂ (@Exp.app α) (@app_resp α)
def app'_mk {α : Type} : (x y : Exp α) → ⟦x.app y⟧ = @app' α ⟦x⟧ ⟦y⟧
  := fun x y => (Quotient.map₂_mk (@Exp.app α) (@app_resp α) x y).symm
...
def eval' {α}
  := Quotient.lift (@eval α) (@eval_lemma2 α)
def eval'_mk {α} : (x : Exp α) → eval x = eval' ⟦x⟧
  := fun x => (Quotient.lift_mk (@eval α) (@eval_lemma2 α) x).symm
...
#print correctness.R_eq
#print eval_lemma1.R_eq
